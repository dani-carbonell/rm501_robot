...
DEFAULT_VEL_MED=5000
DEFAULT_VEL_SLOW=3000
DEFAULT_ACCEL=8000
AXIS1_MIN_LIM=-135
AXIS1_MAX_LIM=135
AXIS2_MIN_LIM=-10
...
class Axis(object):
	def __init__(self, rc, name, addr, m_nr, default_spd=10000, default_acc=19000):
				...
				for waypoint_index in range(self.dialog.tableWidget.rowCount()):
					...
					for axis in range(6):
							...
							if axis is (0):
								...
								if not int(self.dialog.tableWidget.item(waypoint_index,axis).text()) is (0):
									time.sleep(1)
								else:
									time.sleep(0)
