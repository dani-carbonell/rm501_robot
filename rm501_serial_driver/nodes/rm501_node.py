...
DEFAULT_NODE_NAME = "rm501_node"
DEFAULT_LOOP_HZ = 100
DEFAULT_ADDRESS1 = 0x80
DEFAULT_ADDRESS2 = 0x81
DEFAULT_ADDRESS3 = 0x82
DEFAULT_DEADMAN_SEC = 2
DEFAULT_STATS_TOPIC = "~stats"
DEFAULT_SPEED_CMD_TOPIC = "~speed_command"
DEFAULT_POSITION_CMD_TOPIC = "~position_command"
DEFAULT_JOINTSTATES_TOPIC = "~jointstates"
DEFAULT_RATIO_AXIS1 = 105
DEFAULT_RATIO_AXIS2 = 105
DEFAULT_RATIO_AXIS3 = 105
DEFAULT_RATIO_AXIS4 = 105
DEFAULT_RATIO_AXIS5 = 105
...
class RM501Node:
    def __init__(self, node_name):
        ...
        self._rbc_ctl1 = None  # Set by the connect() method below
        self._rbc_ctl2 = None  # Set by the connect() method below
        self._rbc_ctl3 = None  # Set by the connect() method below
        ...
        self._jointstate_pub = rospy.Publisher(
            rospy.get_param("~stats_topic", DEFAULT_STATS_TOPIC),
            Stats,
            queue_size=1
        )
    ...
    def roboclaw_control(self):
        return self._rbc_ctl1

    def connect(self, dev_name, baud_rate, address1, address2, address3, test_mode=False):
        ...
        self._rbc_ctl1 = RoboclawControl(roboclaw, address1)
        self._rbc_ctl2 = RoboclawControl(roboclaw, address2)
        self._rbc_ctl3 = RoboclawControl(roboclaw, address3)
...
if __name__ == "__main__":
    ...
    address1 = int(rospy.get_param("~address1", DEFAULT_ADDRESS1))
    address2 = int(rospy.get_param("~address2", DEFAULT_ADDRESS2))
    address3 = int(rospy.get_param("~address3", DEFAULT_ADDRESS3))
    ...
    ratio_axis1 = int(rospy.get_param("~ratio_axis1", DEFAULT_RATIO_AXIS1))
    ratio_axis2 = int(rospy.get_param("~ratio_axis2", DEFAULT_RATIO_AXIS2))
    ratio_axis3 = int(rospy.get_param("~ratio_axis3", DEFAULT_RATIO_AXIS3))
    ratio_axis4 = int(rospy.get_param("~ratio_axis4", DEFAULT_RATIO_AXIS4))
    ratio_axis5 = int(rospy.get_param("~ratio_axis5", DEFAULT_RATIO_AXIS5))


    rospy.logdebug("node_name: {}".format(node_name))
    rospy.logdebug("dev_name: {}".format(dev_name))
    rospy.logdebug("baud: {}".format(baud_rate))
    rospy.logdebug("address1: {}".format(address1))
    rospy.logdebug("address2: {}".format(address2))
    rospy.logdebug("address3: {}".format(address3))
    ...
    rospy.logdebug("ratio_axis1: {}".format(ratio_axis1))
    rospy.logdebug("ratio_axis2: {}".format(ratio_axis2))
    rospy.logdebug("ratio_axis3: {}".format(ratio_axis3))
    rospy.logdebug("ratio_axis4: {}".format(ratio_axis4))
    rospy.logdebug("ratio_axis5: {}".format(ratio_axis5))

    node = RM501Node(node_name)
    rospy.on_shutdown(node.shutdown_node)

    try:
        # Initialize the RM501 controller
        node.connect(dev_name, baud_rate, address1, address2, address3, test_mode)
